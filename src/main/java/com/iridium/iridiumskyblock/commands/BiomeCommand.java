...
public class BiomeCommand extends Command {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (args.length != 2) {
            player.openInventory(new IslandBiomeGUI(islandOptional.get(), player.getWorld().getEnvironment(), getCooldownProvider(), player.getOpenInventory().getTopInventory()).getInventory());
